# Migrating Expo UI to Verve Design System

Read `artifacts/verve-design-system/docs/AGENTS.md` and
`artifacts/verve-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/verve-design-system/lib/native-theme` and
  `@workspace/verve-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/verve-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/verve-design-system` and retains only
