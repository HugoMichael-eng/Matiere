# Migrating Expo UI to S2 Design System

Read `artifacts/s2/docs/AGENTS.md` and
`artifacts/s2/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/s2/lib/native-theme` and
  `@workspace/s2/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/s2/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/s2` and retains only
