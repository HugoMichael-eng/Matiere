# Consuming S2 Design System in Expo apps

Read `artifacts/s2/docs/AGENTS.md` first. React Native does
...
`artifacts/s2/docs/migrating-expo.md` before writing UI.
...
```tsx
import { tokens } from "@workspace/s2/tokens";
...
```tsx
import { nativeTheme } from "@workspace/s2/lib/native-theme";
import { useColors } from "@workspace/s2/hooks/use-colors";
import { useDesignSystemFonts } from "@workspace/s2/hooks/use-fonts";
```
...
```tsx
import { Badge } from "@workspace/s2/components/native/badge";
import { Button } from "@workspace/s2/components/native/button";
import { Card } from "@workspace/s2/components/native/card";
```
...
Import and render
`@workspace/s2/components/native/button`, then run Expo
