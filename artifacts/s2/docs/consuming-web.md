# Consuming S2 Design System in web apps

Read `artifacts/s2/docs/AGENTS.md` first. This guide covers
...
`artifacts/s2/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/s2/styles.css";
```
...
`node_modules/@workspace/s2/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/s2/components/ui/button";
import { cn } from "@workspace/s2/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/s2/hooks/use-toast";
```
...
`@workspace/s2/components/ui/button`. Run the app's typecheck
