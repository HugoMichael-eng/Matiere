# Consuming S1 Design System in web apps

Read `artifacts/s1/docs/AGENTS.md` first. This guide covers
...
`artifacts/s1/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/s1/styles.css";
```
...
`node_modules/@workspace/s1/src/components` to `content`.
...
import {
  toast,
  useToast,
} from "@workspace/s1/hooks/use-toast";
```
...
`@workspace/s1/components/ui/button`. Run the app's typecheck
