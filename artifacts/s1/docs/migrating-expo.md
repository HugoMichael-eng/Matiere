# Migrating Expo UI to S1 Design System

Read `artifacts/s1/docs/AGENTS.md` and
`artifacts/s1/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/s1/lib/native-theme` and
  `@workspace/s1/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/s1/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/s1` and retains only
