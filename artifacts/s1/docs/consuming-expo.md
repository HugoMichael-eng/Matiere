# Consuming S1 Design System in Expo apps

Read `artifacts/s1/docs/AGENTS.md` first. React Native does
...
`artifacts/s1/docs/migrating-expo.md` before writing UI.
...
```tsx
import { tokens } from "@workspace/s1/tokens";
...
```tsx
import { Badge } from "@workspace/s1/components/native/badge";
import { Button } from "@workspace/s1/components/native/button";
import { Card } from "@workspace/s1/components/native/card";
```
...
Import and render
`@workspace/s1/components/native/button`, then run Expo
